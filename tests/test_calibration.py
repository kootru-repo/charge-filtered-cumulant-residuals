...
def test_u0_baseline_is_floating_point_zero():
    ...
    assert u0["delta_cat"] < 1e-12, f"U=0 baseline failed Sec V: delta_cat = {u0['delta_cat']:.2e}"
...
def test_eta_universal_below_one():
    ...
    rows = _load_calibration()["hubbard"]
    etas = [
        r["eta_universal"] for r in rows if r["eta_universal"] == r["eta_universal"]
    ]  # filter NaN
    assert max(etas) < 0.1, f"eta_universal exceeded 0.1: max={max(etas):.4f}"
